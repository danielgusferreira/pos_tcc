    ...
    public class CategoriaController : Controller
    {
        ...
        public async Task<IActionResult> Index(int codigo, string nome, string descricao)
        {
            try
            {
                ...
                if (codigo != 0)
                {
                    ...
                    using (var httpClient = new HttpClient(httpClientHandler))
                    {
                        using (var response = await httpClient.PutAsync("https://flysneakersbeapi.azurewebsites.net/api/categoria/" + codigo, httpContent))
                        {
                            var resultApi = await response.Content.ReadAsStringAsync();
                ...
                else
                {
                    ...
                    using (var httpClient = new HttpClient(httpClientHandler))
                    {
                        using (var response = await httpClient.PostAsync("https://flysneakersbeapi.azurewebsites.net/api/categoria", httpContent))
                        {
                            var resultApi = await response.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> Excluir(int codigo)
        {
            try
            {
                using (var httpClient = new HttpClient(httpClientHandler))
                {
                    using (var response = await httpClient.DeleteAsync("https://flysneakersbeapi.azurewebsites.net/api/categoria/"+ codigo))
                    {
                        var resultApi = await response.Content.ReadAsStringAsync();
